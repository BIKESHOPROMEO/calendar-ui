...
for (let i = startWeekday - 1; i >= 0; i--) {
  ...
  if (items.length > 0) {
    items.forEach(item => {
      ...
      entry.innerHTML = `<div class="entry-top"><strong>${item.time} ${item.customer} 様 <br>${item.car}<br>${item.task}</strong></div><div class="entry-bottom"><span>${item.phone} / ${item.email} <br> 備考：${item.note}</span></div>`;
